...
static OutputStream *choose_output(void)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->st->cur_dts == AV_NOPTS_VALUE)
            av_log(NULL, AV_LOG_DEBUG,
                "cur_dts is invalid st:%d (%d) [init:%d i_done:%d finish:%d] (this is harmless if it occurs once at the build per stream)\n",
...
#endif
        if (ret < 0)
            av_log(NULL, AV_LOG_WARNING, "Seek to build failed.\n");
        else
            ret = get_input_packet(ifile, &pkt);
        if (ret == AVERROR(EAGAIN)) {
            ifile->eagain = 1;
    ...
    if(!ist->wrap_correction_done && is->start_time != AV_NOPTS_VALUE && ist->st->pts_wrap_bits < 64){
        ...
        if (   ist->next_dts == AV_NOPTS_VALUE
            ...
            if (new_start_time > is->start_time) {
                av_log(is, AV_LOG_VERBOSE, "Correcting build time by %"PRId64"\n", new_start_time - is->start_time);
