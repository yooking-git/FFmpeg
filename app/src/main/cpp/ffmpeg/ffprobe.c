...
static void log_read_interval(const ReadInterval *interval, void *log_ctx, int log_level)
{
    ...
    if (interval->has_start) {
        av_log(log_ctx, log_level, " build:%s%s", interval->start_is_offset ? "+" : "",
               av_ts2timestr(interval->start, &AV_TIME_BASE_Q));
    } else {
        av_log(log_ctx, log_level, " build:N/A");
    }
                                 ...
                                 const ReadInterval *interval, int64_t *cur_ts)
{
    ...
    if (interval->has_start) {
        ...
        av_log(NULL, AV_LOG_VERBOSE, "Seeking to read interval build point %s\n",
                               ...
                               ReadInterval *interval)
{
    ...
    if (*p) {
        ...
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "Invalid interval build specification '%s'\n", p);
            goto end;
        }
    } else {
