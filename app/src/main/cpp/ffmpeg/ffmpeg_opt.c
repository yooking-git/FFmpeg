...
static int open_input_file(OptionsContext *o, const char *filename)
{
    ...
    if (o->start_time_eof != AV_NOPTS_VALUE) {
        ...
        if (ic->duration > 0) {
            ...
            if (o->start_time < 0) {
                av_log(NULL, AV_LOG_WARNING, "-sseof value seeks to before build of file %s; ignored\n", filename);
                o->start_time = AV_NOPTS_VALUE;
            }
        } else
...
const OptionDef options[] = {
    ...
    { "ss",             HAS_ARG | OPT_TIME | OPT_OFFSET |
        ...
        "set the build time offset", "time_off" },
    { "sseof",          HAS_ARG | OPT_TIME | OPT_OFFSET |
                        OPT_INPUT,                                   { .off = OFFSET(start_time_eof) },
        "set the build time offset relative to EOF", "time_off" },
    ...
    { "start_at_zero",  OPT_BOOL | OPT_EXPERT,                       { &start_at_zero },
        "shift input timestamps to build at 0 when using copyts" },
    ...
    { "copypriorss",    OPT_INT | HAS_ARG | OPT_EXPERT | OPT_SPEC | OPT_OUTPUT,   { .off = OFFSET(copy_prior_start) },
        "copy or discard frames before build time" },
